...
Built for [[https://github.com/gdvek/org-cube][org-cube]], usable standalone.  Uses [[https://github.com/karlicoss/orgparse][orgparse]] as the parsing
backend.
...
** From the command line
  ...
  #+begin_src sh
  pip install org-cube-parse   # Python >= 3.11
  #+end_src
  ...
  * DONE Review draft
  ...
  #+begin_src sh
  python -m org_cube_parse example.org
  #+end_src
  ...
  #+begin_src sh
  python -m org_cube_parse --config examples/config.json example.org
  #+end_src
...
** From Python
  ...
  #+begin_src python
  from org_cube_parse import parse_file, Config
...
* Key concepts
  ...
  #+begin_example
          ...
          +--------------+--------------+
          v              v              v
     org-cube       custom indexers   data pipelines
     (DB + UI)     (knowledge graphs)(analytics)
  #+end_example

  orgparse handles the org-mode grammar.  org-cube-parse handles item
  discrimination, field extraction, and content filtering.

* Installation

  #+begin_src sh
  pip install org-cube-parse
  #+end_src
...
* CLI reference
  ...
  =python -m org_cube_parse --help= for the full list.
  ...
  # With TODO keywords
  python -m org_cube_parse --todos TODO,NEXT,DOING --dones DONE,CANCELED file.org
  ...
  python -m org_cube_parse -v file.org
  python -m org_cube_parse --json -vv file.org
  #+end_src
  ...
  #+begin_src sh
  python -m org_cube_parse --config examples/config.json file.org
  #+end_src
...
* Performance
  ...
  Breakdown: orgparse loads the syntax tree in ~1.5 s, org-cube-parse
...
* Development
  #+begin_src sh
  git clone https://github.com/gdvek/org-cube-parse.git
  cd org-cube-parse
  python -m venv .venv
